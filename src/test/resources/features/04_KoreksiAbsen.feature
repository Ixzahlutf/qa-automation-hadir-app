Feature: Correction Feature
  ...
  Scenario: Periksa fungsi tombol reset pada halaman koreksi absen in Aplikasi Hadir.
    ...
    And user menekan ikon jam masuk dan menginput data jam 8 menit 30 untuk tanggal "26" bulan "August" tahun "2025".
    And user menekan ikon jam keluar dan menginput data jam 17 menit 00 untuk tanggal "26" bulan "August" tahun "2025".
    And user menekan tombol reset.

  Scenario: Periksa fungsi tombol kembali pada halaman koreksi absen in Aplikasi Hadir.
    Given user sudah berada di halaman koreksi absen untuk pengecekan tombol kembali.
    When user menekan tombol kembali.
    Then user dibawa kembali ke halaman dashboard dengan url "https://magang.dikahadir.com/apps/absent".